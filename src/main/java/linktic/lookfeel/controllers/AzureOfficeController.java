...
public class AzureOfficeController {
     ...
     * @throws Exception
     */
    @GetMapping(value = "/azure-validate/{email}", produces = "application/json")
    public ResponseEntity<?> azureValidate(HttpServletRequest request, @PathVariable("email") String email) throws Exception {
        String tokenOffice = request.getHeader("token-office");
        if (tokenOffice != null && !tokenOffice.isEmpty()) {
            try {
                this.iAzureOfficeService.getUserInfoAndValidToken(tokenOffice, email);
                ReponseUsuarioLoginDTO usuarioAzureDTO = iAzureOfficeService.getDataUserByEmailOfficeEncriptado(email);
                return ResponseEntity.ok().body(usuarioAzureDTO);
            } catch (Exception e) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(e.getMessage());
            }
        }
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("No tiene permiso para acceder al recurso");
    }
